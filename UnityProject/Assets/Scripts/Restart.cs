...
public class Restart : MonoBehaviour {
    ...
    void Start() {
        ...
        gameObject.transform.position -= new Vector3(-0, -10000, 0);                                      // Move the button to the middle of the screen
    }

    void Update() {

        // if (playerScript.GetComponent<Player>().gas <= 0 && playerScript.GetComponent<Player>().onGround)  // If the player runs out of gas
        //     gameObject.transform.position = new Vector3(0, 15, 1000);                                           // Move the button to the middle of the screen
    }

    public void OnClick(){

        // Restart the game
        //spawnDoodadsScript.GetComponent<SpawnDoodads>().Restart();                                          // Restart the doodads (Not needed atm)
